...

{
...

    {
        ...
        switch (facing)
        {
            case InitialFacing.Up: stateName = "Idle_Back"; break;
            case InitialFacing.Down: stateName = "Idle_Front"; break;
            case InitialFacing.Left: stateName = "Idle_Left"; break;
            case InitialFacing.Right: stateName = "Idle_Right"; break;
        }

        // State names must match the Animator Controller, not animation clip names.
        // When a blend tree handles idle facing, the parameters above select the pose.
        if (stateName != null && animator.HasState(0, Animator.StringToHash("Base Layer." + stateName)))
            animator.Play("Base Layer." + stateName, 0, 0f);
